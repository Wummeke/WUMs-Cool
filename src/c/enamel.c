...
GColor enamel_get_LineColor(){
	Tuple* tuple = dict_find(&s_dict, 3317720342);
...
const char* enamel_get_DateFormat(){
	Tuple* tuple = dict_find(&s_dict, 3245352007);
...
static uint32_t prv_map_messagekey(const uint32_t key){
	if( key == MESSAGE_KEY_LineColor) return 3317720342;
	if( key == MESSAGE_KEY_DateFormat) return 3245352007;
	return 0;
}
...
static void prv_inbox_received_handle(DictionaryIterator *iter, void *context) {
	if(dict_find(iter, MESSAGE_KEY_LineColor)){
		if(s_dict_buffer){
			free(s_dict_buffer);
			s_dict_buffer = NULL;
		}
